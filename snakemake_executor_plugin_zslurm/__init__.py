...
class ExecutorSettings(ExecutorSettingsBase):
    ...
    ip: Optional[str] = field(
        default=None, metadata={"help": "IP address of the zslurm server. Default is localhost."}
    )
...
class Executor(RemoteExecutor):
    ...
    def run_job(self, job: JobExecutorInterface):
        ...
        comment_str = wildcard_str
        job_name = job.name
        partition = job.resources.get("partition", "compute")
        cmd = self.format_job_exec(job)

        reqtime = ensure_float(job.resources.get("time", 3600), f"time must be an number for {job_name}")
        cores = ensure_float(job.resources.get("n",1), f"n must be an number for {job_name}")
        mem = ensure_float(job.resources.get("mem_mb", 500), f"mem_mb must be an number for {job_name}")

        arch_use_add = ensure_float(job.resources.get("arch_use_add", 0), f"arch_use_add must be an number for {job_name}")
        arch_use_remove = ensure_float(job.resources.get("arch_use_remove", 0), f"arch_use_remove must be an number for {job_name}")
        dcache_use_add = ensure_float(job.resources.get("dcache_use_add", 0), f"dcache_use_add must be an number for {job_name}")
        dcache_use_remove = ensure_float(job.resources.get("dcache_use_remove", 0), f"dcache_use_remove must be an number for {job_name}")
        active_use_add = ensure_float(job.resources.get("active_use_add", 0),f"active_use_add must be an number for {job_name}")
        active_use_remove = ensure_float(job.resources.get("active_use_remove", 0), f"active_use_remove must be an number for {job_name}")
        ...
        try:
            ...
            raise WorkflowError(
                "The limit_auto_threads resource must be an integer."
            )
        ...
        while attempt > 0 :
            try:
                ...
                slurm_jobid = s.submit_job(job_name, cmd, cwd, env, 
                                    cores, mem, reqtime, requeue, dependency,
                                    arch_use_add, arch_use_remove, dcache_use_add, 
                                    dcache_use_remove, active_use_add, active_use_remove,
                                    partition, info_input_mb, info_output_file, comment_str)
                break
            except (socket.error, httplib.HTTPException) as serror:
                attempt -= 1
                if attempt == 0:
                    raise WorkflowError(f"ZSLURM submission failed. The error message was {serror}")
        ...
        self.report_job_submission(
            SubmittedJobInfo(
                job, external_jobid=slurm_jobid
            )
        )
    ...
    async def check_active_jobs(
        ...
        try:
            ...
            self.logger.error(f"ZSLURM job status check failed. The error message was {serror}")
        ...
        for active_job in active_jobs:
            ...
            async with self.status_rate_limiter:
                ...
                if state in done_states:
                    ...
                    if done:
                        self.report_job_error(active_job, msg=f"Job {active_job.external_jobid} has ended with an unknown state {state}. Assuming failure. Update the ZSlurm executor.")
                    else:
                        self.logger.warning(f"Job {active_job.external_jobid} has an unknown state {state}. Update the zslurm executor.")
                        yield active_job
